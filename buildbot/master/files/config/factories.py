...
windows = ServoFactory([
    ...
    steps.Compile(command=["bash", "-l", "-c", "cd /c/buildbot/slave/windows/build ; ./mach build -d -v"],
                  env=envs.build_windows),
    steps.Compile(command=["bash", "-l", "-c", "cd /c/buildbot/slave/windows/build ; ./mach test-unit"],
                  env=envs.build_windows),
